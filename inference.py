...
from src.utils.preprocess import CropAndExtract, CropAndExtract2
...
def main(args):
    ...
    if pic_path.split('.')[-1] in ['jpg', 'png', 'jpeg']:
        preprocess_model = CropAndExtract2(path_of_net_recon_model, dir_of_BFM_fitting, device)
    else:
    ...
    # coeff2video
    data = get_facerender_data(pic_path, coeff_path, crop_pic_path, first_coeff_path, audio_path, batch_size, device,
                               args.size)
    tmp_path, new_audio_path, return_path = animate_from_coeff.generate(data, save_dir, pic_path, crop_info,
                                                                        restorer_model, enhancer_model, enhancer_region,args.preprocess)
